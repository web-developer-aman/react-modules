...
export default function EditPost(){
    ...
    const updatePost = async (e) => {
        ...
        Array.from(e.target.elements).forEach((input) => {
            if(!input.value.trim() && input.name == 'title' || !input.value.trim() && input.name == 'categories'){
                ...
                post['created_at'] = new Date().toLocaleString('en-US', { month: 'short', day: 'numeric' });
                post['tags'] = tags;
        ...
        if (Object.keys(errors).length === 0) {
            ...
            try {
                ...
                body: JSON.stringify({ ...post, image: featureImg}),
                });
    ...
    return (
        <div className='max-w-[1240px] md:mx-auto mx-3'>
            <ToastContainer />
            <h1 className='text-3xl font-bold text-center my-5'>Edit Post</h1>
                ...
                <form onSubmit={(e) => updatePost(e)}>
                    ...
                    <div className='mb-5 w-[200px]'>
                        <label htmlFor='imageInput'>
                            <img
                                src={featureImg || post.image || imageNotFound}
                                alt='Feature Image'
                    ...
                    <div className='mb-3'>
                        ...
                        <input id='title' type="text" defaultValue={post.title} name='title' className='border rounded-lg p-3 w-full focus:outline-none'/>
                        {errors.title && <p className='text-red-500 pt-1'>{errors.title}</p>}
                    </div>
                    <div className='mb-3'>
                        <label htmlFor="description" className='block text-gray-700 text-sm font-bold mb-2'>Description</label>
                        <textarea id='description' defaultValue={post.description} name='description' className='border rounded-lg p-3 w-full focus:outline-none' rows='3'></textarea>
                    </div>
                    <div className='mb-3'>
                        <label htmlFor="category" className='block text-gray-700 text-sm font-bold mb-2'>Category</label>
                        <select name="categories" defaultValue={post.categories} id="categories" className='border rounded-lg p-3 w-full focus:outline-none'>
                            {
                    ...
                    <div className='flex justify-end'>
                        <button className='bg-blue-500 py-2 px-5 rounded text-white'>Update Post</button>
                    </div>
                </form>
            </div>
        </div>
